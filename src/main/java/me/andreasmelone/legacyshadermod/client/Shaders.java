...
import me.andreasmelone.legacyshadermod.mixin.EntityRenderDispatcherAccessor;
import me.andreasmelone.legacyshadermod.mixin.LivingEntityRendererAccessor;
...
public class Shaders {
    ...
    private static final ByteBuffer bigBuffer = (ByteBuffer) ((Buffer) BufferUtils.createByteBuffer(1812)).limit(0);
    ...
    private static Pattern gbufferFormatPattern = Pattern.compile("[ \t]*const[ \t]*int[ \t]*(\\w+)Format[ \t]*=[ \t]*([RGBA81632F]*)[ \t]*;.*");
    private static Pattern gbufferMipmapEnabledPattern = Pattern.compile("[ \t]*const[ \t]*bool[ \t]*(\\w+)MipmapEnabled[ \t]*=[ \t]*true[ \t]*;.*");
    ...
    public static void init() {
        if (!isInitialized) {
            ...
            boolean isMcpatcherLoaded;
            try {
                Class.forName("com.prupe.mcpatcher.hd.MipmapHelper");
                isMcpatcherLoaded = true;
            } catch (Exception e) {
                isMcpatcherLoaded = false;
            }
            if (isMcpatcherLoaded) {
                SMCLog.warning("Mcpatcher detected! This may be buggy!");
            }
            ...
            ((Buffer) dfbColorTextures).position(0).limit(8);
            ...
            for (int p = 0; p < 22; p++) {
                if (p == 20) {
                    ...
                    if (str != null) {
                        ...
                        for (int ix = 0; ix < numDB; ix++) {
                            int d = 0;
                            if (str.length() > ix) {
                                int ca = str.charAt(ix) - '0';
                                if (p != 21) {
                                    if (ca >= 0 && ca <= 7) {
                                        d = ca + 36064;
            ...
            ((Buffer) dfbColorTextures).position(0).limit(usedColorBuffers);
            ...
            for (int ix = 0; ix < 22; ix++) {
                ...
                if (n != ix && ix != 21) {
                    programsID[ix] = programsID[n];
                    programsDrawBufSettings[ix] = programsDrawBufSettings[n];
                    programsDrawBuffers[ix] = programsDrawBuffers[n];
                }
            }
    ...
    private static int createVertShader(String filename) {
        ...
        if (vertShader == 0) {
            ...
            ARBShaderObjects.glShaderSourceARB(vertShader, vertexCode.toString());
    ...
    private static void printLogInfo(int obj, String name) {
        ...
        if (compiled == GL11.GL_FALSE) SMCLog.warning("Shader failed to compile!");
        int length = iVal.get();
        if (length > 1) {
    ...
    public static void useProgram(int program) {
        ...
        if (activeProgram != program) {
            ...
            if (programsID[program] == 0) {
                ...
                setProgramUniform1f("aspectRatio", (float) renderWidth / renderHeight);
    ...
    private static void setupFrameBuffer() {
        ...
        GL11.glGenTextures((IntBuffer) ((Buffer) dfbColorTextures).clear().limit(usedColorBuffers));
    ...
    public static void beginRender(MinecraftClient minecraft, float f, long l) {
        if (!isShadowPass) {
            ...
            ShadersTex.bindNSTextures(((IShaderTexture) defaultTexture).shadermod$getMultiTexID());
            useProgram(2);
    ...
    public static void setCamera(float f) {
        ...
        invertMat4x((FloatBuffer) ((Buffer) projection).position(0), (FloatBuffer) ((Buffer) projectionInverse).position(0));
        ((Buffer) projection).position(0);
        ((Buffer) projectionInverse).position(0);
        GL11.glGetFloat(2982, (FloatBuffer) ((Buffer) modelView).position(0));
        invertMat4x((FloatBuffer) ((Buffer) modelView).position(0), (FloatBuffer) ((Buffer) modelViewInverse).position(0));
        ((Buffer) modelView).position(0);
        ...
        if (isShadowPass) {
            ...
            invertMat4x((FloatBuffer) ((Buffer) shadowProjection).position(0), (FloatBuffer) ((Buffer) shadowProjectionInverse).position(0));
            ...
            invertMat4x((FloatBuffer) ((Buffer) shadowModelView).position(0), (FloatBuffer) ((Buffer) shadowModelViewInverse).position(0));
    ...
    static {
        ...
        ((Buffer) drawBuffersColorAtt0.put(36064)).position(0).limit(1);
    }
}
